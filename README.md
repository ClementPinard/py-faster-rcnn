Real time object recognition with a Parrot Jumping Sumo drone.
...
Refer to the video to see the project in action:

[![Youtube video](http://forthtemple.com/sumo/youtube.jpg)](https://youtu.be/WDaA7Qkv0xo)


Images are sent from the sumo via wifi to a PC which has an NIVIDIA GPU which processes the image real time using Faster RCNN image classification. This project is a fork of py-faster-rcnn and includes an extra ardrone directory and model directory. The processed images are displayed and recorded on the PC. The sumo is also controlled from the PC.

This project uses an original caffemodel that was trained for four classes including glasses, mugs, shoes and apples. The training process was done with over 600 images.
